...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        rand_val = random.random()*sum(self.values())
        total = 0
        for key, val in self.items():
            total += val
            if rand_val <= total:
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for pos in self.allPositions:
            ...
            genProb = self.beliefs[pos]
            self.beliefs[pos] = obsProb * genProb
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.allPositions:
            ...
            for newPos, prob in newPosDist.items():
                new[newPos] += self.beliefs[oldPos] * prob
        self.beliefs = new
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        self.particles = []
        "*** YOUR CODE HERE ***"#wait check q5
        particlesPerPosition = int(self.numParticles / len(self.legalPositions))

        for pos in self.legalPositions:
            for i in range(particlesPerPosition):
                self.particles.append(pos)
        #compensate the trucated decimals
        diff = self.numParticles - (particlesPerPosition*len(self.legalPositions))
        for i in range(diff):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                ghostDis = observation[i]
                jailPos = self.getJailPosition(i)
                if ghostDis is not None:
                    likelihood *= self.getObservationProb(ghostDis, pacPos, particle[i], jailPos)
                else:
