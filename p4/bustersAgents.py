    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"#wait check q4

        minDist = None
        for dis in livingGhostPositionDistributions:
            guessPos = dis.argMax()
            distance = self.distancer.getDistance(pacmanPosition, guessPos)
        ...
        for act in legal:
            ...
            diff = self.distancer.getDistance(newPos, nearestPos)
            if not minDist or diff < minDist:
                minDist = diff
                bestAct = act
